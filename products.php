<?php
session_start();




?>

<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>Products - AgroConnect</title>

  <!-- Bootstrap CSS -->
  <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.2/dist/css/bootstrap.min.css" rel="stylesheet">

  <!-- Custom CSS -->
  <style>
    body {
      font-family: 'Poppins', sans-serif;
      background-color: #f8f9fb;
    }

    .navbar {
      background-color: #28a745;
    }
    .navbar-brand {
      font-weight: 700;
      color: white !important;
    }
    .nav-link {
      color: white !important;
      margin-left: 20px;
    }
    .nav-link:hover {
      text-decoration: underline;
    }

    .products-section {
      padding: 100px 20px 60px;
    }
    .products-section h2 {
      text-align: center;
      font-weight: 700;
      margin-bottom: 50px;
    }

    .card {
      border-radius: 15px;
      box-shadow: 0 3px 12px rgba(0,0,0,0.1);
      transition: transform 0.3s, box-shadow 0.3s;
    }
    .card:hover {
      transform: translateY(-5px);
      box-shadow: 0 6px 20px rgba(0,0,0,0.15);
    }
    .card img {
      height: 200px;
      object-fit: cover;
      border-top-left-radius: 15px;
      border-top-right-radius: 15px;
    }
    .card-title {
      font-weight: 600;
    }
    .card-text {
      color: #28a745;
      font-weight: 500;
    }
    .btn-contact {
      background-color: #28a745;
      color: white;
      border: none;
      border-radius: 10px;
      padding: 8px 15px;
      transition: 0.3s;
    }
    .btn-contact:hover {
      background-color: #1e7e34;
    }

    footer {
      background-color: #28a745;
      color: white;
      text-align: center;
      padding: 20px 0;
      margin-top: 60px;
      font-size: 0.9rem;
    }
  </style>
</head>
<body>

  <!-- Navbar -->
  <nav class="navbar navbar-expand-lg navbar-dark fixed-top">
    <div class="container">
      <a class="navbar-brand" href="index.html">AgroConnect</a>
      <button class="navbar-toggler" type="button" data-bs-toggle="collapse" data-bs-target="#navMenu">
        <span class="navbar-toggler-icon"></span>
      </button>
      <div class="collapse navbar-collapse" id="navMenu">
        <ul class="navbar-nav ms-auto">
          <li class="nav-item"><a class="nav-link" href="index.html">Home</a></li>
          <li class="nav-item"><a class="nav-link active" href="#">Products</a></li>
          <li class="nav-item"><a class="nav-link" href="registration.html">Register</a></li>
          <li class="nav-item"><a class="nav-link" href="contact.html">Contact</a></li>
        </ul>
      </div>
    </div>
  </nav>

  <!-- Products Section -->
  <section class="products-section">
    <div class="container">
      <h2>Available Produce</h2>
      <div class="row g-4">

        <!-- Product 1 -->
        <div class="col-md-4">
          <div class="card">
            <img src="https://via.placeholder.com/400x250?text=Corn" class="card-img-top" alt="Corn">
            <div class="card-body text-center">
              <h5 class="card-title">Fresh Corn</h5>
              <p class="card-text">$20 / Bag</p>
              <p>Farmer: John Doe</p>
              <button class="btn-contact">Contact Farmer</button>
            </div>
          </div>
        </div>

        <!-- Product 2 -->
        <div class="col-md-4">
          <div class="card">
            <img src="https://via.placeholder.com/400x250?text=Tomatoes" class="card-img-top" alt="Tomatoes">
            <div class="card-body text-center">
              <h5 class="card-title">Tomatoes</h5>
              <p class="card-text">$15 / Basket</p>
              <p>Farmer: Amina Bello</p>
              <button class="btn-contact">Contact Farmer</button>
            </div>
          </div>
        </div>

        <!-- Product 3 -->
        <div class="col-md-4">
          <div class="card">
            <img src="https://via.placeholder.com/400x250?text=Yams" class="card-img-top" alt="Yams">
            <div class="card-body text-center">
              <h5 class="card-title">Yams</h5>
              <p class="card-text">$25 / Tuber</p>
              <p>Farmer: Paul N.</p>
              <button class="btn-contact">Contact Farmer</button>
            </div>
          </div>
        </div>

        <!-- Product 4 -->
        <div class="col-md-4">
          <div class="card">
            <img src="https://via.placeholder.com/400x250?text=Carrots" class="card-img-top" alt="Carrots">
            <div class="card-body text-center">
              <h5 class="card-title">Carrots</h5>
              <p class="card-text">$10 / Kg</p>
              <p>Farmer: Mary Johnson</p>
              <button class="btn-contact">Contact Farmer</button>
            </div>
          </div>
        </div>

        <!-- Product 5 -->
        <div class="col-md-4">
          <div class="card">
            <img src="https://via.placeholder.com/400x250?text=Peppers" class="card-img-top" alt="Peppers">
            <div class="card-body text-center">
              <h5 class="card-title">Bell Peppers</h5>
              <p class="card-text">$12 / Kg</p>
              <p>Farmer: Samuel K.</p>
              <button class="btn-contact">Contact Farmer</button>
            </div>
          </div>
        </div>

        <!-- Product 6 -->
        <div class="col-md-4">
          <div class="card">
            <img src="https://via.placeholder.com/400x250?text=Spinach" class="card-img-top" alt="Spinach">
            <div class="card-body text-center">
              <h5 class="card-title">Spinach</h5>
              <p class="card-text">$8 / Bunch</p>
              <p>Farmer: Grace O.</p>
              <button class="btn-contact">Contact Farmer</button>
            </div>
          </div>
        </div>

        <!-- Product 7 -->
        <div class="col-md-4">
          <div class="card">
            <img src="./edu.PNG" class="card-img-top" alt="Potatoes">
            <div class="card-body text-center">
              <h5 class="card-title">Potatoes</h5>
              <p class="card-text">$18 / Bag</p>
              <p>Farmer: David L.</p>
              <button class="btn-contact">Contact Farmer</button>
            </div>
          </div>
        </div>

        <!-- Product 8 -->
        <div class="col-md-4">
          <div class="card">
            <img src="https://via.placeholder.com/400x250?text=Onions" class="card-img-top" alt="Onions">
            <div class="card-body text-center">
              <h5 class="card-title">Onions</h5>
              <p class="card-text">$14 / Bag</p>
              <p>Farmer: Fatima S.</p>
              <button class="btn-contact">Contact Farmer</button>
            </div>
          </div>
        </div>

      </div> <!-- row -->
    </div> <!-- container -->
  </section>

  <footer>
    <p>© 2025 AgroConnect | Built with Ayo❤️ for Farmers</p>
  </footer>

  <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.2/dist/js/bootstrap.bundle.min.js"></script>
</body>
</html>
